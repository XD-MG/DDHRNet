...
from models.modeling import ddhrnet
...
def seg_model(image, sar_image, class_num):
    model_name = cfg.MODEL.MODEL_NAME
    if model_name == 'ddhrnet':
        # 输入的是两张不同模态的图，分别具有两个尺度
        # logits = shibinnet.shibinnet(RGB_image1x, RGB, SARimage, class_num)
        logits = ddhrnet.ddhrnet(image, sar_image, class_num)
    return logits
...
def build_model(main_prog, start_prog, phase=ModelPhase.TRAIN):
    ...
    with fluid.program_guard(main_prog, start_prog):
        with fluid.unique_name.guard():
            ...
            sar_image = fluid.data(name='sar_image', shape=image_shape, dtype='float32')
            ...
            if ModelPhase.is_train(phase) or ModelPhase.is_eval(phase):
                data_loader = fluid.io.DataLoader.from_generator(
                    feed_list=[image, sar_image, label, mask],
            ...
            cfg.PHASE = phase
            logits = seg_model(image, sar_image, class_num)
